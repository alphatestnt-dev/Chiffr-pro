(()=>{
  ...
  function style(){
    ...
    s.textContent=`
      ...
      @media(max-width:560px){
        .ce-page-nav{margin-bottom:12px}
        .ce-page-nav button{padding:8px 12px;font-size:14px}
      }
  ...
  function hideExtraNav(){
    document.querySelectorAll('nav[aria-label="Navigation principale"] button').forEach(b=>{
      const id=b.dataset.p;
      const keep=primary.some(x=>x[0]===id);
      b.style.display=keep?'':'none';
    });
    const nav=document.querySelector('nav[aria-label="Navigation principale"]');
    if(!nav) return;
    primary.forEach(([id,label])=>{
      const b=nav.querySelector(`button[data-p="${id}"]`);
  ...
  function makeSubnav(pageId, items){
    ...
    bar.className='ce-page-nav';
    bar.setAttribute('aria-label',`Navigation ${pageId}`);
  ...
  function addTools(){
    ...
    if(eco&&!eco.querySelector('.ce-tools-nav')){
      ...
      const card=eco.querySelector('.card');
      if(card)card.appendChild(wrap);
    }
    const param=$('param');
    if(param&&!param.querySelector('.ce-tools-nav')){
      const oldPartner=$('partnerNav');
      const wrap=document.createElement('div');wrap.className='ce-tools-nav';
      const b=document.createElement('button');b.type='button';b.textContent='🤝 Partenaires';b.onclick=()=>window.go?.('partnersPage');wrap.appendChild(b);
      const card=param.querySelector('.card');if(card)card.appendChild(wrap);
      if(oldPartner)oldPartner.style.display='none';
    }
  }
  ...
  function install(){
    ...
    if(home){
      home.src='./hero-home-ecopro.svg';
      home.removeAttribute('srcset');
      home.style.imageRendering='auto';
    }
  }
